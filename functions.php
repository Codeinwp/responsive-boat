...
function zerius_customize_register( $wp_customize ) {
	...
	$wp_customize->add_section(
		new Ti_Notify(
			...
			'ti-notify',
			array(
				'text'     => sprintf( __( 'This child theme is not maintained anymore, consider using the parent theme %1$s or check-out our latest free one-page theme: %2$s.','zerius' ), sprintf( '<a href="https://wordpress.org/themes/zerif-lite/" target="_blank">%s</a>', 'Zerif Lite' ), sprintf( '<a href="https://wordpress.org/themes/hestia/" target="_blank">%s</a>', 'Hestia' ) ),
				'priority' => 0,
			)
		)
	);
...
function zerius_admin_notice() {
	...
	if ( is_admin() && ( 'themes.php' == $pagenow ) && isset( $_GET['activated'] ) ) {
		...
		printf( __( 'This child theme is not maintained anymore, consider using the parent theme %1$s or check-out our latest free one-page theme: %2$s.','zerius' ), sprintf( '<a href="https://wordpress.org/themes/zerif-lite/" target="_blank">%s</a>', 'Zerif Lite' ), sprintf( '<a href="https://wordpress.org/themes/hestia/" target="_blank">%s</a>', 'Hestia' ) );
		echo '</p></div>';
	}
}
